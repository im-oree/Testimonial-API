<!-- DOC 6 — THE IMMUNE SYSTEM. Fetched verbatim from GitHub repo `im-oree/Testimonial-API`, branch `main`, file `next.md` (SHA 9b5358cac4b6b03a55be3003968683726597d76f, 57340 bytes, commit afc4c6c "Update next.md") via `gh api` on 2026-09-08. Status legend: ✅ verified in-sandbox · ⬜ needs browser/device/CI/external runner · 🔶 partial. Doc 3/4/5 convention: verbatim body, then progress log at the bottom; checklists are the doc's own. -->
...
### 2026-09-08 — §A SQL-injection-prevention slice (user pick: "start working on ot")
**Gate numbers:** `npm run lint` (api) 0 errors · `tsc` api typecheck 0 · api vitest **141/141** (14 files; +72 new security tests) · `turbo run lint` 8/8 · `turbo run typecheck` 10/10 · `turbo run test` 9/9 (api 141, domain 13, ui 10) · check-doc1 167/0 · check-doc2 87/0 · check-doc4 189/0 · check-doc5 214/0 (no regressions).

**Landed (Doc 6 §1):**
- **§1.1/§1.2 ESLint raw-SQL boundary** — `packages/config/eslint/base.js`: `$queryRawUnsafe` **globally banned** (`no-restricted-syntax`, selector `CallExpression[callee.property.name='$queryRawUnsafe']`) and a `**/postgres/repositories/**` override banning `sql\`…\`` tags, `Prisma.sql\`…\`` tags, and bare `$queryRaw` calls. Positive-proof on real code + negative-proof: probe files in a repo path and in `src/` each failed lint with the exact rule messages (probes deleted afterwards).
- **§1.3 sanitizer** — `apps/api/src/common/utils/sanitize-search-input.ts` (escape `% _ \`, strip `<> ' " ;`, cap 200 — doc's exact order), wired into `postgres-testimonial.repository.ts` `buildWhere` before every Prisma `contains`/ILIKE filter (search + authorName). Write path untouched: stored content is never escaped/stripped.
- **§1.5 redaction** — `apps/api/src/common/utils/secret-redaction.util.ts` `redactConnectionString` (`postgres(ql)://…@` → `***@`), applied to the global exception filter's server-side log line and `prisma.client.ts` connection-failure log. Client-facing 500 body was already generic (no SQL/DSN text) — now pinned by tests.
- **§1.6 DB hardening artifact** — `infra/postgres/hardening/01-security-hardening.sql` (idempotent, psql-run): least-privilege `testimonial_app` role (no superuser/CREATE; password set out-of-band), RLS + `tenant_isolation` policy on `testimonials` via `current_setting('app.current_tenant_id', true)`, dangerous-extension lockdown (dblink/file_fdw/postgres_fdw/plpython*/plperl/lo/pg_repack), `log_ddl` event trigger → `security_audit.ddl_log`. `infra/postgres/MIGRATION.md` gained "Step 5.5 — Security hardening (pre-cutover)".
- **§1.7 adversarial suite** — `apps/api/test/security/sql-injection.spec.ts` (61 tests): the doc's 15 payloads verbatim × search / tags / create paths, run at the Postgres repository seam with a fake Prisma delegate (public HTTP routes are still Doc-3 placeholders), plus error-envelope tests driving `AllExceptionsFilter` with a fake `ArgumentsHost`. Assertions: never throws (no 500-equivalent), empty results, tenant `app_id` scoping survives every payload, filters carry only the sanitized literal, tags travel as parameterized array values, created payloads store **verbatim** (mapper round-trip identity), and no response/error text matches `/syntax error|pg_|relation|column/i` or leaks a DSN. Supporting unit suites: `sanitize-search-input.test.ts` (6) and `secret-redaction.test.ts` (5).
- **Boundary exemption (test files)** — `packages/config/eslint-plugin-repo-boundaries`: `no-db-adapter-outside-db-infra` now exempts `test/` + `*.spec.ts` files, mirroring the existing AI-rule exemption (Doc-6 §1.7 needs adapter-internal fakes; src-side boundary unchanged).

**Checklist status (doc's A1–A8 verbatim checklist above stays ⬜ — mapped evidence here):**
- A1 ✅ in-sandbox: ESLint repo override live + `grep` shows zero string-concatenation/raw-SQL queries in `postgres/repositories/` (all 21 files Prisma query-builder).
- A2 ✅ in-sandbox (rule live; adapter exception documented): grep finds no `$queryRawUnsafe` call sites anywhere except the single sanctioned, eslint-disabled pass-through inside `prisma.client.ts` (Doc 6 §1.2 disable path w/ justification) + interface/stub declarations. Positive lint proof on probes.
- A3 ✅ in-sandbox: `$queryRaw`/`$executeRaw` appear **zero times** outside the adapter type surface; code review of every occurrence = none to review.
- A4 ✅ in-sandbox: sanitizer + §1.7 payload suite confirms zero results, not errors, at the repository seam. Live-DB row check stays ⬜ (no running Postgres).
- A5 ⬜ live DB — artifact created (`hardening/01-security-hardening.sql`) but role-grant inspection requires the running database.
- A6 ⬜ live DB — RLS policy artifact created; cross-tenant query proof requires the running database.
- A7 ⬜ CI — suite exists and passes locally (61 tests); no CI workflow runs it yet (Doc 6 §8/§I future slice).
- A8 ⬜ live DB / ⬜ partial in-sandbox ✅: filter test proves a deliberately-thrown SQL-looking error yields the generic 500 envelope with **no** SQL details or DSN; note the repo-wide error contract (README §11, Doc 1) uses `code: 'INTERNAL'`, whereas the Doc-6 checklist prints `INTERNAL_ERROR` — no SQL-text leak is the hard requirement and is what the suite pins; live-DB syntax-error trigger stays ⬜.

**Deferred to later slices (noted, not silently dropped):** §1.7 HTTP-e2e run once Doc-3 public routes land (`/v1/public/testimonials` search/tags/create), CI wiring for the suite, remaining §2+ vector slices (XSS/CSRF/SSRF/IDOR/mass-assignment/path-traversal/XXE/open-redirect/DoS/supply-chain/prompt-injection), §3 auth hardening, §4–§5 infra, §6 pentest, §7 k6, §8 CI/CD, §9 DR, §10 compliance, and the §L Sign-Off Gate.
